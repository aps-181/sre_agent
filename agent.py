...
agent = Agent(
    model=local_model, output_type=InvestigationResult, system_prompt=SRE_SYSTEM_PROMPT
)
...
async def main():
    ...
    server_params = StdioServerParameters(
        command="python", args=["-m", "app.mcp.server"], env=safe_env
    )
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            for t in mcp_tools.tools:
                ...
                params = [
                    f"{p_name}: {p_info.get('type', 'str')}"
                    for p_name, p_info in schema_props.items()
                ]
                params_str = f"({', '.join(params)})" if params else "()"
                tool_descriptions_list.append(
