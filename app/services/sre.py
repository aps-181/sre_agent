...
def fetch_service_logs(service_name: str) -> str:
    print(
        f"[Server] Tool called: fetch_service_logs for '{service_name}'",
        file=sys.stderr,
    )
    mock_logs = {
    ...
    return mock_logs.get(
        service_name, f"No logs found for {service_name}. Service might be healthy."
    )


def fetch_db_metrics(database_name: str = "payments_db") -> str:
    print(
        f"[Server] Tool called: fetch_db_metrics for '{database_name}'", file=sys.stderr
    )
